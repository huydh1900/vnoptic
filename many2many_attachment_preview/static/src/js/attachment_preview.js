...
export class Many2ManyAttachmentPreview extends Component {
    ...
    getOfficeViewerUrl(fileId) {
        const fileUrl = window.location.origin + this.getUrl(fileId);
        return "https://view.officeapps.live.com/op/embed.aspx?src=" +
            encodeURIComponent(fileUrl);
    }

    // many2many_attachment_preview.js

    async previewDocx(fileId) {
        try {
            // Lấy dữ liệu file
            const response = await fetch(this.getUrl(fileId));
            const arrayBuffer = await response.arrayBuffer();

            // Tạo một vùng chứa để hiển thị
            const container = document.createElement("div");
            container.classList.add("docx-viewer-container");
            container.style.backgroundColor = "white";

            // 2. Gọi render (docx-preview sẽ lấy window.JSZip để chạy loadAsync)
            await window.docx.renderAsync(arrayBuffer, container);

            // Hiển thị nội dung ra đâu đó, ví dụ dùng Dialog của Odoo
            this.displayPreview(container);
        } catch (error) {
        }
    }
