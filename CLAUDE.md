...
All commands run from repository root. Replace `<odoo.conf>` and `<db>` with your local values.
...
odoo-bin -c <odoo.conf> --addons-path=/home/huytq/vnoptic/vnoptic -d <db> --test-enable --test-tags <module_name> --stop-after-init
...
- **vnop_sync** — Core module. Syncs lenses, frames, accessories from an external Spring Boot API (`https://erp.vnoptictech.com.vn`). Handles product master data, Excel import/preview, and server connector configuration. Credentials and API settings live in `.env`.
- **vnop_contract** — Contract and contract line management with document type classification.
- **vnop_delivery** — Delivery schedule planning and OTK (Order To Keep) logistics tracking with wizard operations.
- **vnop_purchase** — Purchase order extensions. Depends on `vnop_delivery` and `vnop_contract`.
- **vnop_purchase_offer** — Purchase quotation/proposal system. Depends on `vnop_contract`.
- **vnop_currency_rate** — Auto-syncs currency rates from Vietcombank/SBV via cron.
- **vnop_sale** — Sales order customizations.
- **vnop_partner** — `res.partner` extensions.
- **vnop_theme** — Custom login UI with SCSS/JS assets.
- **vnop_chatter_toggle** — Collapsible form chatter (JS frontend module).
...
**Module dependency chain**: `vnop_purchase` → `vnop_delivery` → `vnop_contract`; `vnop_purchase_offer` → `vnop_contract`.
